...
export async function invalidToken(){
    ...
    const promise = await supertest(app).get("/disciplines").send(body).set("Authorization", "um_token_bem_errado");
...
export async function valid(){
    ...
    const promise = await supertest(app).get("/disciplines").set("Authorization", login.body.token);
