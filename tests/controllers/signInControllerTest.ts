...
export async function incorrectEmail(){
    ...
    const result = await supertest(app).post("/sign-in").send({ email:"incorrect" + user.email, password: user.password});
...
export async function incorrectPassword(){
    ...
    const result = await supertest(app).post("/sign-in").send({ email: user.email, password: "incorrect" + user.password});
...
export async function invalidEmail(){
    ...
    const result = await supertest(app).post("/sign-in").send({ email:"", password: user.password});
...
export async function invalidPassword(){
    ...
    const result = await supertest(app).post("/sign-in").send({ email:user.email, password:""});
